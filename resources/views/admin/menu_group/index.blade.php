...
<main class="content">
				<div class="container-fluid p-0">
					...
					<h1 class="h3 mb-3">Menu Groups List</h1>
							...
							<div class="card">
								...
								<div class="table-responsive card-body">
									<div class="row">
										...
										<div class="col-md-2">
											<a href="{{ route('admin.menu_group.create') }}" class="btn btn-success"> <i class="fa fa-plus"></i> Add Menu Groups</a>
										</div>
										...
										<tbody>
											@foreach ($data as $key => $val)
												<tr>
													...
													<td>
														<a href="{{ route('admin.menu_group.edit',$val->id) }}" class="btn btn-primary">Edit</a>
														<a href="javascript:;" onclick="destroy('{{$val->id}}')" class="btn btn-danger">Delete</a>
														<form method="post" action="{{ route('admin.menu_group.destroy',$val->id) }}" id="menu_group{{$val->id}}">
															@csrf
															@method('DELETE')
														</form>
													</td>
		...
		function destroy(id){
			...
			Swal.fire({
			  ...
			  if (result.isConfirmed) {
			  	document.getElementById(`menu_group${id}`).submit();
			  }
			})
		}
	</script>
