				...
				<div class="card">
					...
					<div class="card-body">
						...
						@endif
							...
							@csrf
							<input type="hidden" name="restro_request_id" value="{{ $data->id ?? '' }}">
							...
							<div class="row">
								<div class="mb-4 col-md-6">
									...
									<input type="text" name="name" class="form-control" placeholder="enter restaurant name" value="{{ old('name',$restaurant->name ?? $data->restaurant_name) }}">
									@error('name')
										<span class="text-danger">{{$message}} </span>
									@enderror
								</div>
								...
								<div class="mb-4 col-md-6">
									...
									<input type="text" name="address" class="form-control" placeholder="enter address"  value="{{ old('address',$restaurant->address ?? $data->address) }}">
									@error('address')
										<span class="text-danger">{{$message}} </span>
									@enderror
								</div>
								...
								<div class="mb-4 col-md-6">
									...
									<input type="text" name="city" class="form-control" placeholder="enter city" value="{{ old('city',$restaurant->city ?? $data->city) }}">
									@error('city')
										<span class="text-danger">{{$message}} </span>
									@enderror
								</div>
								<div class="mb-4 col-md-6">
									...
									<select name="state" class="form-control">
										...
										<option value="{{$kii}}" @if($restaurant !=null && $restaurant->state==$kii || old('state',$val==$data->state)==$kii) selected="" @endif>{{$val}}</option>
										@endforeach
									</select>
									@error('state')
										<span class="text-danger">{{$message}} </span>
								...
								<div class="mb-4 col-md-6">
									...
									<input type="text" name="zipcode" class="form-control" placeholder="enter zipcode" value="{{ old('zipcode',$restaurant->zipcode  ?? $data->zipcode) }}">
									@error('zipcode')
										<span class="text-danger">{{$message}} </span>
									@enderror
								</div>
							...
							<div class="row">
								...
								<div class="mb-4 col-md-4">
									...
									<select class="form-control" name="status" >
										<option value="1" {{$restaurant!=null && $restaurant->status == 1 ? 'selected':'' }} >Active</option>
										<option value="0" {{$restaurant!=null && $restaurant->status == 0 ? 'selected':'' }} >Inactive</option>
									</select>
							...
							<button type="submit" class="btn btn-primary">Send Details to Vendor</button>
						</form>
					</div>
				</div>
			</div>
...
@push('script')
	...
	<script type="text/javascript">
		...
		@if (Session::has('success'))
			...
			Swal.fire({
	  			icon: 'success',
	  			title: 'Successfully! Send Details to Vendor'
			})
